...
// persistent flags, infrastructure.go resolves the harness driver the flags name and the store that keeps
// exchange records,
// domain.go builds the session domain and mounts its commands, scenarios.go mounts the
// narrated scenarios and their listing, and commands.go composes the mounts. app.go holds the
// application itself: New assembles the layers without I/O, and Run executes the command
